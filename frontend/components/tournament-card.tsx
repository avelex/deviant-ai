...
export type TournamentStatus = 'ACTIVE' | 'REGISTRATION' | 'FINISHED';
...
export function TournamentCard({ id, title, status, mainIcon, category, mode, slots, timeLabel, timeValue, reward }: TournamentCardProps) {
  ...
  const TimeIcon = isActive ? Timer : isRegistration ? Calendar : Flag;
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        (isActive || isRegistration) && "hover:border-[#00E5FF] dark:hover:border-[#00E5FF]",
        isFinished && "opacity-75 grayscale-[50%] hover:grayscale-0"
      )}
    >
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 h-full relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-8 w-full lg:w-auto mb-2 lg:mb-0">
          ...
          <div className={cn(
            ...
            isActive && "bg-[#131b2e] dark:bg-slate-800 text-white",
            isRegistration && "border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400",
            isFinished && "border border-slate-200 dark:border-slate-800 bg-transparent text-slate-500 dark:text-slate-500"
          )}>
            <MainIconComponent
              size={28}
              className={cn(
                isActive ? 'text-[#00E5FF] fill-[#00E5FF]' : isRegistration ? 'text-slate-400 dark:text-slate-500' : 'text-slate-500 dark:text-slate-400'
              )}
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col md:flex-row md:items-center gap-3">
              <span className={cn(
                ...
                isActive && "bg-[#00E5FF] text-black",
                isRegistration && "bg-indigo-50 dark:bg-cyan-950/20 border border-slate-200 dark:border-slate-800 text-[#131b2e] dark:text-slate-300",
                isFinished && "bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400"
              )}>
                {status}
              </span>
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 text-[11px] md:text-[12px] font-semibold tracking-widest uppercase text-slate-500 dark:text-slate-400">
              ...
              <div className="flex items-center gap-2 text-[#131b2e] dark:text-slate-200">
                <TimeIcon size={14} className={isActive ? 'text-[#00E5FF]' : 'text-slate-400 dark:text-slate-500'} />
                <span className={cn(isActive ? 'text-[#00E5FF]' : 'text-[#131b2e] dark:text-slate-200')}>
        ...
        <div className="flex flex-row lg:flex-col items-center lg:items-end w-full lg:w-auto border-t lg:border-t-0 pt-6 lg:pt-0 border-slate-100 dark:border-slate-800/50 justify-between lg:justify-start">
          <div className="flex flex-col lg:items-end">
            ...
            <div className={cn(
              ...
              (isActive || isRegistration) ? "text-[#00E5FF]" : "text-slate-400 dark:text-slate-500"
            )}>
              {reward}
            </div>
          </div>
