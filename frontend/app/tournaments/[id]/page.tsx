...
import { ChevronLeft, Check, FileText, X, Settings } from "lucide-react";
...
export default function TournamentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // We are using the hardcoded mockData to match the requested design perfectly,
  // but normally we would fetch the specific tournament by 'id' here.
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-slate-200 dark:border-slate-800 pb-8 gap-6 md:gap-0">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 px-4 py-2 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-[10px] font-bold tracking-widest uppercase">
                  ...
                  <span className="text-[#131b2e] dark:text-white">{mockData.status}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
            ...
            <div className="lg:col-span-2 flex flex-col gap-6 lg:gap-8">
               ...
               <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 md:p-8">
                 ...
                 {/* Chart Graph Area */}
                 <div className="relative w-full h-[220px] md:h-[260px] border border-slate-100 dark:border-slate-800 overflow-hidden bg-[linear-gradient(to_right,#e2e8f080_1px,transparent_1px),linear-gradient(to_bottom,#e2e8f080_1px,transparent_1px)] dark:bg-[linear-gradient(to_right,#33415580_1px,transparent_1px),linear-gradient(to_bottom,#33415580_1px,transparent_1px)] bg-[size:24px_24px]">
                   <div className="absolute top-4 left-4 text-[10px] font-bold tracking-widest text-slate-400 dark:text-slate-500 uppercase z-10 bg-white/60 dark:bg-slate-900/60 px-2 py-1 backdrop-blur-sm">
                     VOLATILITY INDEX (SIMULATED)
                   </div>
                   {/* Simulated fluid wave layers using SVG */}
                   <svg className="absolute bottom-0 w-full h-full preserve-3d" preserveAspectRatio="none" viewBox="0 0 1000 240">
                     <path d="M0,240 L0,180 C300,190 400,100 1000,130 L1000,240 Z" fill="#00E5FF" fillOpacity="0.08" />
                     <path d="M0,240 L0,210 C400,240 600,140 1000,100 L1000,240 Z" fill="#00E5FF" fillOpacity="0.12" />
                     <path d="M0,240 L0,230 C200,225 350,210 500,190 C700,165 850,150 1000,145 L1000,240 Z" fill="#00E5FF" fillOpacity="0.1" />
                   </svg>
                 </div>
               </div>
               ...
               <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 md:p-8">
                 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 border-b border-slate-100 dark:border-slate-800/50 pb-4">
                   ...
                   <div className="text-[10px] font-bold tracking-widest uppercase text-[#00E5FF] bg-[#00E5FF]/10 px-3 py-1.5 self-start sm:self-auto border border-[#00E5FF]/20">
                     TOTAL POOL: 45,000 USDC
                   </div>
                 </div>
                 
                 <div className="flex flex-col gap-2">
                   {mockData.roster.players.map((player, idx) => {
            ...
            <div className="flex flex-col gap-6 lg:gap-8">
               ...
               <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 md:p-8 pt-6">
                 ...
                 <div className="flex flex-col gap-5">
                   ...
                   {(() => {
                     const [currentSlots, maxSlots] = mockData.roster.filledDisplay.split(' ')[0].split('/').map(Number);
                     const isNotFull = currentSlots < maxSlots;
                     const isRegistrationOpen = mockData.status === "REGISTRATION OPEN";
            ...
            <div className="p-6 overflow-y-auto flex-1">
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                   <div className="border border-slate-100 dark:border-slate-800/50 p-4 shrink-0 shadow-sm shadow-slate-100/50 dark:shadow-none bg-slate-50/50 dark:bg-slate-900/50">
                     ...
                     <div className="text-lg text-[#131b2e] dark:text-white font-medium">{mockData.parameters.category}</div>
                   </div>
                   <div className="border border-slate-100 dark:border-slate-800/50 p-4 shrink-0 shadow-sm shadow-slate-100/50 dark:shadow-none bg-slate-50/50 dark:bg-slate-900/50">
                     <div className="text-[10px] font-bold tracking-widest text-slate-400 dark:text-slate-500 uppercase mb-2">Type</div>
                     <div className="text-lg text-[#131b2e] dark:text-white font-medium">{mockData.parameters.type}</div>
                   </div>
                   <div className="border border-slate-100 dark:border-slate-800/50 p-4 shrink-0 shadow-sm shadow-slate-100/50 dark:shadow-none bg-slate-50/50 dark:bg-slate-900/50">
                     <div className="text-[10px] font-bold tracking-widest text-slate-400 dark:text-slate-500 uppercase mb-2">Duration</div>
                     <div className="text-lg text-[#131b2e] dark:text-white font-medium">{mockData.parameters.duration}</div>
                   </div>
                   <div className="border-y border-r border-slate-100 dark:border-slate-800/50 border-l-[3px] border-l-[#00E5FF] p-4 bg-white dark:bg-slate-900 shadow-sm shadow-slate-100/50 dark:shadow-none">
                     ...
                     <div className="text-lg text-[#131b2e] dark:text-white font-bold">{mockData.parameters.reward}</div>
                   </div>
                 </div>
            </div>
